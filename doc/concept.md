...
| --- | --- | --- | --- | --- |
| Capture (Service) | Generate raw image sequence, control animation state. | FrameSequencer, RepaintBoundary <sup>2</sup>, GlobalKey. | Dart dart:ui image manipulation, in-memory raw RGBA streaming. | High-fidelity frame buffers streamed to FFmpeg. |
| --- | --- | --- | --- | --- |
| Encoding (Data/Infra) | Executes external media processing commands and manages filter graph complexity. | FFmpegFilterGraphBuilder, VideoEncoderService. | Process.run (Dart), FFmpeg CLI. | Final Encoded Video File (e.g., MP4). |
| --- | --- | --- | --- | --- |
...
final SequenceConfig? child; // For nested sequences
//...
}  
...
FFmpeg acts as the composition engine, executed via Dart's process APIs. To support features like layering and blending, the system relies entirely on **complex filtergraphs** (-filter_complex), which are necessary for handling multiple input streams and outputs.<sup>8</sup>

Rather than staging PNG files, the Flutter-generated frame stream (Input 0) is piped directly into FFmpeg as raw RGBA bytes. All supplementary media are composited onto or beneath this canvas using spatial filters like overlay and audio filters like amix.

### D. Encoding Quality Controls

`RenderConfig.encoding` encapsulates per-render quality decisions. A `RenderQuality` enum (low/medium/high/lossless) maps to sensible CRF and FFmpeg preset pairs (30/veryfast, 23/medium, 18/slow, 0/veryslow respectively). Advanced callers can override the CRF or preset directly (e.g., forcing CRF 10 with preset `faster`) without touching the widget tree, ensuring reproducible renders with deterministic encoder settings.
...
The FrameSequencer service, coordinated with the main isolate, iteratively controls the global state. It waits for Flutter's paint pipeline (`SchedulerBinding.addPostFrameCallback`) and captures each frame as raw RGBA bytes before streaming them to FFmpeg:
...
final rawBytes = await mainThreadPort.receive('capture_frame_rgba');  
<br/>// 3. Stream bytes directly into FFmpeg stdin  
ffmpegSink.add(rawBytes);  
...
| --- | --- | --- | --- |
| Sequence | (Equivalent to content within &lt;Sequence&gt;) | Foundational visual container, applies spatial properties and timing. **Note:** Renamed from "Clip" in v0.1.0 for clarity. | Implements the Renderable interface to output a SequenceConfig.<sup>10</sup> |
| --- | --- | --- | --- |
...
| --- | --- | --- | --- | --- |
| **LayerStack** | **(New Feature)** Enables arbitrary, layered composition (Z-index). | Maps directly to a Flutter Stack widget. The visual layering is resolved entirely by Flutter's painting phase and captured in the single Input 0 raster stream. | None (Pre-composited by Flutter). | **Yes** (New) |
| --- | --- | --- | --- | --- |
...
| --- | --- | --- | --- |
| **TextSequence** | **(New Feature)** Renders standard, high-quality, animated text. | Standard Flutter Text widget within the Sequence. Utilizes Flutter's font rendering for precise kerning and anti-aliasing. | Rasterized as part of the main Input 0 frame stream, avoiding the complexity of FFmpeg's drawtext filter.<sup>11</sup> |
| --- | --- | --- | --- |
...
- **Input Registration:**
  - Register the Flutter frame stream (Input 0) fed via stdin: \[0:v\] demuxer_options \[canvas\].
...
- **Preparation (Main Isolate):** Extract RenderConfig from the widget tree and determine the pixel ratio required to hit the requested TimelineConfig resolution.
- **Capture & Streaming:** FrameSequencer captures each frame as raw RGBA bytes after `SchedulerBinding.addPostFrameCallback` confirms the scene is painted. The bytes are streamed directly into the encoder's stdin, eliminating temporary PNG files.
- **Encoding:** VideoEncoderService maintains a single long-lived `Process.start('ffmpeg', ...)` session, configured through `EncodingConfig` (quality, CRF override, preset override). The stdin pipe is flushed once all frames are written, and FFmpeg finalizes the MP4 output.
- **Completion:** Await FFmpeg's exit code and update RenderStateNotifier to Status.Complete; any failure triggers session cancellation and error propagation.
...
- **Unit Tests (Domain & Infra):** Verify the mathematical correctness of Drivers (SpringDriver, VideoTween <sup>4</sup>), the streaming semantics of `VideoEncoderService` (argument mapping, error propagation, quality overrides), and the structural output of FilterGraphBuilder classes (FilterNode, FilterChain).
...
| **Scenario** | **Given** | **When** | **Then** |
| --- | --- | --- | --- |
| **ColorFilter Effect** | I apply a VintageEffect to a Sequence. | The FFmpegFilterGraphBuilder processes the RenderConfig. | The output filtergraph includes a curves filter applied to the sequence's stream label, referencing the vintage preset or a custom curve string.<sup>12</sup> |
| --- | --- | --- | --- |
