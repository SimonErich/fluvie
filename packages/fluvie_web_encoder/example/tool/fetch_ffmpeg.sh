#!/usr/bin/env bash
# Vendor the ffmpeg.wasm files this example serves from web/ffmpeg/.
#
# The wasm core is tens of megabytes, so it is fetched here instead of being
# committed. Run this once before `flutter run -d chrome`. Needs npm on PATH.
set -euo pipefail
...
# Pin the same versions the package's end-to-end test verifies.
