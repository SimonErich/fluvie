// WI-28 (D15): the PhotoFrame micro-defaults, structure-pinned here and pixel-
...
void main() {
  group('PhotoFrame (D15, §14)', () {
    testWidgets('none is a pure passthrough', (tester) async {
      await tester.pumpWidget(const Center(child: PhotoFrame.none(child: _child)));
    ...
    testWidgets('rounded clips at its radius (default 24)', (tester) async {
      await tester.pumpWidget(const Center(child: PhotoFrame.rounded(child: _child)));
      final clip = tester.widget<ClipRRect>(find.byType(ClipRRect));
      expect(clip.borderRadius, BorderRadius.circular(24));
    });

    testWidgets('rounded forwards an explicit radius', (tester) async {
      await tester.pumpWidget(const Center(child: PhotoFrame.rounded(radius: 8, child: _child)));
      final clip = tester.widget<ClipRRect>(find.byType(ClipRRect));
    ...
    testWidgets('card: white surface, radius 16, padding 12, one deterministic shadow', (
      ...
      await tester.pumpWidget(const Center(child: PhotoFrame.card(child: _child)));
      final decoration =
    ...
    testWidgets('polaroid: square white border 12/12/12/48 with the same shadow', (tester) async {
      await tester.pumpWidget(const Center(child: PhotoFrame.polaroid(child: _child)));
      final decoration =
          tester.widget<DecoratedBox>(find.byType(DecoratedBox)).decoration as BoxDecoration;
      expect(decoration.color, const Color(0xFFFFFFFF));
    ...
    testWidgets('card and polaroid wrap the child tightly (no expansion)', (tester) async {
      await tester.pumpWidget(const Center(child: PhotoFrame.card(child: _child)));
      expect(tester.getSize(find.byType(PhotoFrame)), const Size(60 + 24, 40 + 24));
      await tester.pumpWidget(const Center(child: PhotoFrame.polaroid(child: _child)));
      expect(tester.getSize(find.byType(PhotoFrame)), const Size(60 + 24, 40 + 60));
    });
  });

  group('PhotoFrame Phase 8 params (WI-10, §15)', () {
    testWidgets('polaroid renders the caption text in the bottom border', (tester) async {
      await tester.pumpWidget(
        const Center(
          child: PhotoFrame.polaroid(caption: 'Summer', child: _child),
        ),
      );
    ...
    testWidgets('card forwards radius and elevation to the surface', (tester) async {
      await tester.pumpWidget(
        const Center(child: PhotoFrame.card(radius: 24, elevation: 16, child: _child)),
      );
      final decoration =
          tester.widget<DecoratedBox>(find.byType(DecoratedBox)).decoration as BoxDecoration;
    ...
    test('card defaults are unchanged (radius 16)', () {
      const frame = PhotoFrame.card(child: _child);
    ...
    testWidgets('withChild re-issues the same style with a new child', (tester) async {
      ...
      final framed = const PhotoFrame.card(child: _child).withChild(newChild);
    ...
    testWidgets('withChild preserves a polaroid caption', (tester) async {
      const newChild = SizedBox(key: Key('new'), width: 10, height: 10);
      final framed = const PhotoFrame.polaroid(caption: 'Trip').withChild(newChild);
      await tester.pumpWidget(Center(child: framed));
